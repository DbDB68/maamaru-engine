...
_SCROLLBAR_BOTTOM_Y = 689                 # 滑块到底时底缘 y（钳在轨底，
                                          # 阈值 180~235 读数恒定 689）
_SCROLLBAR_BOTTOM_TOL = 2                 # 贴底容差。离底一页的过渡帧实测
                                          # 685（cal_page_28，内容仍在变），
                                          # 差 4px——容差给 6px 会把它误判成
                                          # 底（2026-09-14 真机反例）
...
class FormationEditorMixin:
    ...
    def _list_end_sighted(self):
        ...
        该页内容仍在变），差 4px，故容差收紧到 2px——宽到 6px 会把
        过渡帧误判成底（同日真机反例）。残余盲区：末页若短到剩余滚动
        ≤2px（不足一行高的零头，该行在候选页视口内已基本完整可见），
        过渡帧会贴底通过——此时未扫内容不足一行，OCR 在候选页已读过它，
        实际危害为零；除此之外「差一行以上」必然差 ≥4px，拦得住。
        """
