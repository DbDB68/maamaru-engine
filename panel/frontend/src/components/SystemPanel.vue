...
const selected = ref<'ai' | 'qq' | 'telegram' | 'broadcast' | 'appearance'>('ai')
...
async function load() {
  [ai.value, bot.value] = await Promise.all([api.chatConfig(), api.botConfig()])
