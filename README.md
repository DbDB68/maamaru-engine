...
成绩单会按每次任务汇总完成圈数、从开工到最后一圈的实际用时、平均圈速、手入、加速符和补刀装。大阪城等循环任务可以选择 1 小时、6 小时、8 小时或自定义时长估算圈数；开工与收工库存都识别成功时，还会展示本轮资源变化和小判每小时收益。

まあ丸把三本账分开保存：

- **狐狸账**：只记流程能够确认完成的出阵、远征、手入、领奖等任务事实。
- **库存账**：只比较两次盘点实际识别到的小判、资源和符，不猜变化来自哪里。
- **审神者报备**：玩家只需选择大概时间和做过什么，不用手填资源数字，也不会反过来改写前两本账。

如果两轮任务之间的库存发生变化，成绩单会把这段差值单独提醒出来，不塞进任何一轮挂机收益。玩家可以补一句“领过邮箱”“手动领奖”，也可以不作说明，只保留差值事实。

![まあ丸 V2 库存差值与审神者报备](docs/assets/v2-report-ledgers.png)

收工盘点偶尔因回城加载失败时，可以先单独运行一次“库存快照”，再为**最近一轮**补上收工数据。这个功能只适合挂机结束后没有领奖、锻刀、手入、购买或其他人工操作的情况；数据一旦被中间操作污染，就不应补回挂机成绩。
...
- **演练先看旧战绩再挑人**：读取当前刷新场已有的胜负章，把已有胜场计入目标，再从尚未挑战的对手中选择目标。
...
数据地基与分账版成绩单已经完成：まあ丸会悄悄记下任务完成情况、大阪城进度、演练胜负、远征、锻刀、手入、活动掉落和库存快照等结构化记录，不再从中文日志里“猜”发生了什么。狐狸账、库存账和审神者报备彼此独立，库存差值不会被强行归因到某次任务。
...
- 已发布版本（v0.2.2）里，run 级开工库存盘点会在一键日课打开游戏／登录之前触发，导致模拟器冷启动登录失效；修复已合入主分支，随下个 release 发布后生效
- 独立「库存快照」脚本因脚本注册参数签名错位，单独运行会报“Agent 没有 status_snapshot_stream 方法”，待后续修复
