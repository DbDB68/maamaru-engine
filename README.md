...
まあ丸是《刀剑乱舞 ONLINE》国服本丸管家。它既能照看日课、远征、出阵和活动，也能在完全不连接游戏的情况下作为一间独立本丸账房使用。

我玩刀剑乱舞九年了，现在还是想玩，只是不想再亲自点完每一轮重复操作，也不想继续手拉 Excel 才知道资源从哪来、花到哪去。所以我做了这个：不是替审神者决定怎么玩，而是把决定之后那些费手、费眼、费计算器的事接过去。
...
![まあ丸正常管家模式：任务、日志、管家小报和家底](docs/assets/v050-dashboard.png)

左边选任务，中间决定目标、部队和停止条件；开始后日志会持续说明它正在做什么。右侧的管家小报只报最近有没有顺利收工、跑了几圈、拿回了什么，不要求审神者翻一页流水找结论。

### 2. 收工以后，先看结论，再看证据

![本丸成绩单：三条结论、手账与资源变化](docs/assets/v050-report.png)

成绩单第一页最多挑三件真正值得知道的事：这段时间完成了什么、哪天变化异常、哪种资源进出最明显。需要追账时再点“看证据”，下钻到对应日期、资源来源、任务和手动说明。能够确认的才归因，无法确认的变化仍留作未知。

### 3. 不开模拟器，也可以只把它当本丸账房

![不连接游戏的纯净本丸账房](docs/assets/v050-ledger.png)

启动器里的“只打开账房”不连接模拟器、ADB、MAA、远征排班或游戏脚本，只保留成绩单、活动规划和自己的手账。第一次使用会引导抄当前家底、可选带回旧账，再立一个真正想盯的目标。

![Excel 账本导出、旧账预览与自动备份入口](docs/assets/v050-ledger-transfer.png)

Excel 可带走完整流水、当前家底、每日汇总和一张可再次导入的表；旧账写入前必须先预览，冲突需要明确确认，真正导入前自动备份。まあ丸确认的流水只能导出，旧表不能反向改写它。

另外提供 Android 离线试玩版：打开就是黄色随身账房，可以手动记收支、家底、活动目标和刀剑男士等级。它没有联网权限，数据只存在手机里，目前不会与电脑账房联动。

## 两种入口，同一个本丸账本

| 入口 | 适合什么时候 | 会做什么 |
|---|---|---|
| **启动まあ丸** | 今天要跑日课、远征、出阵或活动 | 接管重复操作，照看伤势与停止条件，收工后把可确认结果写进成绩单 |
| **只打开账房** | 只想盘家底、补记手动活动、算目标或整理旧 Excel | 完全不连接游戏，只开放成绩单、规划、手账和账本进出 |

两种入口读取同一份本机账本，但まあ丸任务与审神者手动记录始终分开计数、分开求圈速。所有配置、成绩单和备份都留在本机用户数据目录。

> [!CAUTION]
> 本项目与游戏运营方无关。正常管家模式包含游戏自动操作，可能违反游戏服务条款，并带来账号处罚、误操作或数据损失等风险；纯净账房不会连接游戏。请自行判断是否使用，并先保管好重要配置与账本备份。

> [!NOTE]
> **项目状态：持续维护中**
>
> 正常管家模式主要支持 MuMu 12 与 1280×720 游戏画面，已经过维护者长期实机运行，也有非技术用户完成安装和日常使用。游戏更新、新地图、不同模拟器或分辨率仍可能影响画面识别；第一次使用新任务或更换运行环境时，建议先短跑确认伤势处理和停止条件。
...
| **大阪城挖地** | 可指定已解锁的 1～99F 连续挂机；未指定时不选择楼层，只按游戏当前流程持续点击“行军”。支持阵形、伤势处理、手入续跑、补刀装和小判收益实验记录 |
| **江户城潜入调查** | 智能跑 E4：按剩余步数踩点、取钥匙并前往王点；可选择部队、阵形、出阵次数和是否补充手形，并把每圈钥匙记进成绩单。其他难度不计划适配 |
...
| **炼糖** | 从收件箱领取刀剑并循环习合；习合页只会点击列表中的第一把刀。开始前请在游戏中保持开启“隐藏无法习合的刀剑”；上锁刀无法被一键选择，达到安全上限后停止 |
| **收杂物箱** | 只领取收件箱中的资源、货币、便利道具和其他物品；刀剑邮件留在箱中，不会顺手收走 |
...
- 自定资源目标可以选择“攒到多少”或“到哪一天”；狐之助根据近期普通收支估算达成日期或到期数量。
- 活动日程按进行中和即将开始排列；官方公告暂时拉取失败时会保留本地旧日程，并明确标记没有更新。
- 江户城会结合钥匙目标、场均钥匙、免费手形和活动结束时刻估算补票小判；大阪城可以按希望收工时留下的小判库存反推剩余层数和工时。
- 活动实测按活动期次保存，复刻时不会把不同届的数据直接混在一起；没有实测时会如实显示待估算。

成绩单分为 **概览 / 全部记录**：

- 概览可按 24 小时、7 天或 30 天查看：24 小时会并排展示七种日常资源及各自来源（甲州金保留在账上但不塞进日常变化图），7 天和 30 天按资源查看趋势并可下钻到当天来源和任务。
- 本周小结最多挑三条值得看的结论，异常日、进账冠军和明显支出都能直达对应证据，不用先读完整张表。
- 资源变化汇总当前余额与时间段涨跌，并显示账目可信程度；狐之助小结只使用能够确认的来源，不硬猜。
- 掉落结果汇总出阵、活动和锻刀中成功识别的刀剑男士；从“入手 N 振”旁边可以直接设置心愿刀，纯净账房也有独立的“心愿刀”页。命中时本丸小结会额外点名提醒，没有认出名字的记录不会冒充掉落。
- 全部记录通过日历查看最长一年的历史；任务记录可展开轮内活动，无所属任务的活动才单独显示，避免重复。
- 流程确认的收支、库存实际变化和玩家补充说明分开保存。无法解释的差值会留在账上，不会强行塞进某一轮任务。
- 可以手动补记玩家自己打的玩法、圈数和起止时间；手动记录与まあ丸任务分开计数，圈速也不会混合求平均。
- “我的手账”可以修改或撤销手动收支、家底和活动；Excel／CSV 支持导出，旧账导入前先预览冲突并自动备份。
...
- `maamaru-ledger-demo-v*.apk`：Android 离线试玩版，只做手动账房，不连接游戏或电脑端数据。

打开启动器后可以选择两种入口：点击“启动まあ丸”接管游戏；只想记家底、报备自己打的圈数或安排活动目标时，点击“只打开账房”，不需要启动模拟器或连接 ADB／MAA。使用自动化前请阅读 [まあ丸使用说明](docs/user-manual.md)，至少确认模拟器、分辨率、部队、资源消耗、伤势条件和保护名单。
...
源码开发版默认使用 `%LOCALAPPDATA%\Maamaru-Dev`，避免与安装版的配置和成绩单混用。自动化面板使用本机 `8080` 端口；纯净账房会使用独立本机端口，遇到 QQ 等程序占用时自动换到空闲端口。安装版和源码版仍不建议同时运行。

在源码仓库中可以直接双击 `启动源码版.cmd`。它会打开源码版启动器，不再把开发记录写进安装版账本。
...
- 江户城只做智能 E4，其他难度不计划适配；首次使用、游戏改版或活动复刻后应先短跑观察。补充手形仍依赖补票弹窗识别，无法确认时会安全收工。
...
- 根据真实使用反馈继续收拾纯净账房：优先修看不懂、对不上或不好纠正的地方，不为了填路线图继续堆企业 BI；
- 收束资源记账技术债：玩法只负责留下业务事实，并通过统一 `resource.change` 流水申报明确的资源收支；库存快照负责核对余额，统计器不再继续堆每种玩法的专用翻译分支；
- 收束截图与视觉识别技术债：模板采集统一使用脚本实际运行的无损截图，固定识别分辨率与颜色语义，小字状态优先 OCR，并为识别失败保留截图后端、运行帧和 ROI 证据；
- 下次联队战复刻开放时，把当前独立的选队、伤势检查、手形补充和出阵确认编排接入通用安全出阵链，并用真实活动画面回归“不补票 / 小判补一张 / 补完再次出阵”三条路径；
...
为什么都是牛？因为 Codex 的桌宠叫 **NULL**，读快了很像“牛”。
