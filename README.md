...
**选任务 → 定部队、停止条件和资源许可 → 开工。**需要固定阵容时，还可以先套用部队预设。
...
- **出阵与活动**：普通合战场、异去第一章、大阪城、江户城 E4、秘宝之里、联队战（含海边之阵）与南瓜大作战；各玩法保留自己的圈数、阵容、伤势和消耗含义。
- **刀帐与部队预设**：只读盘点所持刀剑，同名多振分别留档；出发前可按预设安排六个位置和指定装备。人选、锁定状态或换装结果认不清时会停下。
- **安全照看**：重伤拦截、伤势停止、手入续跑、补刀装、自动换队长、中断恢复、看门狗和明确的结束动作。
- **成绩单**：汇总出阵、活动、锻刀、掉落、资源变化与异常日；心愿刀入手时额外提醒，没认出名字时不冒充成功。
- **玩法规划**：按本丸记录试算已支持活动的圈数、工时与小判预算；秘宝之里可自定本期玉目标，联队战可查看累计夜光贝。资源目标附「去哪弄」获取途径。
...
- 部队预设与换装仍在试用，请先用容易复原的队伍短跑查看结果，暂时不要交给无人看守的重要队伍。
