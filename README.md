...
这是一场**人类 × AI 结对 vibe coding 的实验成果展**：鬼点子来自人类（刀剑乱舞老玩家，负责脑洞、素材、测试和"要不再加个……？"），落地全部交给 AI（负责把每一个"能……吗？"变成"能"）。从第一行代码到像素面板，全程没有一个字是手敲的。

开源它有两层意思：

1. **可以直接用**——下载、双击、挂机等收菜，开箱程度还算凑合
2. **更推荐：丢给你自己的 Agent 魔改**——代码结构、注释、日志文案都按"AI 读得懂、改得动"的标准写的。如果你想让自己的 AI 学着做一个游戏自动化项目，这份仓库就是现成的教材。我们开源的本意正是这个：**好想法值得被抄走，好代码值得被 AI 们传阅**

至于售后——没有。我们不是客服，不包教包会（忙，在打游戏）。
如果看完你的想法是"我草这也行？"，那就对了，这就是我们挂出来的原因。🦊
