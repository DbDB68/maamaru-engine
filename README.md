...
> [!IMPORTANT]
> **まあ丸是辅助工具，不是“代替人类玩游戏”的机器人。** 它只能在已经实现并经过识别确认的流程里帮忙点击、判断和记账；不能理解所有游戏画面，也不能保证处理网络波动、临时活动改版或未知弹窗。
>
> “本丸成绩单”本质上和用 Notion 或 Excel 拉表统计一样：まあ丸在干活时顺手记录自己能够确认完成的事情，再替你汇总。它不是游戏官方数据，更不会读取账号后台，只是OCR识别；识别不到、没有埋点或由人类亲自完成的行为，都可能不在统计中。
...
> [!WARNING]
> **近侍聊天、LLM 接入、QQ 与 Telegram相关能力目前仍属实验性功能，维护者尚未完成完整实机测试。** 它们不影响本体自动化任务；需要消息渠道或聊天功能时，请把它们当作可选试验功能，不要默认其稳定性与主流程相同。
...
- 成绩单只统计まあ丸能够确认的结构化事件，不能覆盖玩家手动操作或尚未埋点的玩法
- 成绩单统计演练时暂无法认出胜利场次。
...
- 本丸成绩单与收益估算是本地辅助统计，不是游戏官方记录，也不能替代玩家核对账号、资源和任务状态。
