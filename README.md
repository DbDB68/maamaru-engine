...
我玩刀剑乱舞九年了。现在还是想玩，只是日课、远征、活动和重复出阵已经无聊到不想再点了。但更多的是我嫌拉Excel算资源太麻烦了，我本人又特别想知道资源到底是怎么来的怎么没的。

所以我做了这个。它会接手重复操作，也会把活动进度、资源变化和运行效率整理成可以用来规划的信息：现在打到哪、照这个速度还能拿多少、离目标还差多少、剩下的时间够不够，尽量不用再靠我自己掐着日历和计算器算。

具体来说，まあ丸可以执行日课、远征、出阵和活动任务，也会确认任务是否真的完成、在部分中断后尝试恢复，并把能够确认的资源变化和掉落结果记进本丸成绩单。无法确认的事情会保留为未知，不替我猜结果。**并且绝对保留玩家最基础的决策能力，它只是给出数据建议，相当于帮忙拉更清晰易懂的Excel**
<details>
<summary>后续计划</summary>

不依赖自动化的纯净账房入口已经跑通：保留本丸家底、活动规划、手动战绩和资源流水，不启动模拟器、排班或游戏脚本。接下来按[本丸账房后续路线](docs/advanced-excel-roadmap.md)补齐手账纠错、导入导出和首次引导，再决定是否拆成独立仓库。
...
- 概览可按 24 小时、7 天或 30 天查看：24 小时会摊开八种资源及各自来源，7 天和 30 天按资源查看趋势并可下钻到当天来源和任务。
...
- 继续收拾已经跑通的纯净账房入口：补齐纯手动资源流水、表格导入导出与第一次使用引导；麻麻露自动跑出的账与审神者自己打的账继续分开，再按真实使用体验决定是否拆成独立仓库；
